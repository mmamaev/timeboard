...
class TestIntervalCount:
    ...
    def test_interval_count_on(self):
        clnd = tb_12_days()
        for locs in ((2,11), (4, 11), (2,10), (4,10)):
            assert Interval(clnd, locs).count() == 3

    def test_interval_count_off(self):
        clnd = tb_12_days()
        for locs in ((1, 10), (2, 10), (1, 9), (2, 9)):
            assert Interval(clnd, locs).count(duty='off') == 6

    def test_interval_count_any(self):
        clnd = tb_12_days()
        assert Interval(clnd, (1,9)).count(duty='any') == 9

    def test_interval_count_single(self):
        clnd = tb_12_days()
        for loc, duty in ((2, 'off'), (3, 'any'), (4,'on'), (7, 'any')):
            assert Interval(clnd, (loc, loc)).count(duty=duty) == 1

    def test_interval_count_no_such_duty(self):
        clnd = tb_12_days()
        assert Interval(clnd, (2, 3)).count() == 0
        assert Interval(clnd, (4, 4)).count(duty='off') == 0
